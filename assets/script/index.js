const cards = document.querySelector('.cards'); //получаем елемент, в который будем добавлять карточки
...
const supersObj = JSON.parse(supers); // десериализуем JSON, чтобы дальше с ним работать

let i = 0;
let r = 0;

// циклом отрисовываем все карточки
supersObj.forEach(el => {
    cards.innerHTML += `<div class="card-item"><div class="card flipper">
        <div class="front card-body">
            <h2 class="bold name">${el.name}</h2>
            <p class="bold universe">Вселенная: <span>${el.universe}</span></p>
            <p class="bold alterego">Альтерэго: <span>${el.alterego}</span></p>
            <p class="bold occupation">Род деятельности: <span>${el.occupation}</span>
            </p>
            <p class="bold friends">Друзья: <span>${el.friends}</span></p>
            <p class="bold superpower">Суперсила: <span>${el.superpowers}</span></p>
            <img src="${el.url}"
                alt="superhero" class="avatar">
            <button class="more-info">Больше информации</button>
            <div class="showcase">
                <div class="rating-system">
                    <h3>Оцените супергероя</h3>
                    <input type="radio" name='rate${r}' id="star${i}" />
...
const btns = document.querySelectorAll('button'); // получаем все кнопки с карточек

btns.forEach(el => { //проходим по кнопкам циклом
    el.addEventListener('click', () => { // при клике

        const parent = el.parentNode; // получаем родительский элемент кнопки
        const grandParent = parent.parentNode; // получаем родительский элемент родителя кнопки
        const frontCard = grandParent.querySelector('.front'); // получаем лицевую сторону карточки, на которой была нажата кнопка
        const backCard = grandParent.querySelector('.back'); // получаем обратную сторону карточки, на которой была нажата кнопка
        if(el.classList.contains('btn-back')) { // если кнопка включает определенный класс, то показываем одну сторону и прячем другую 
            frontCard.style.display = 'block';
            backCard.style.display = 'none';
        } else { // и обратно, при нажатии кнопки с другим классом
            frontCard.style.display = 'none';
            backCard.style.display = 'block';
        }
    });
});
// что по рейтингу? тут всё плохо
let ratingSys = document.querySelectorAll('.rating-system'); // получаем все элементы, внутри которых располагаются сердечки рейтинга
let rates = document.querySelectorAll(`[type='radio']`); // получаем каждое сердечко
let f = 0;
let rateArr = [];

let arr = localStorage.getItem('rate'); // если в локал у нас есть значения под ключом, то добавляем их в массив с результатами оценки
if(arr != null) {
    rateArr.push(arr);
}
            
rates.forEach(el => { 
    el.addEventListener('click', () => { // циклом идем по сердечкам, и при клике, если айди кликнутого элемента в массиве нет - добавляем в массив, затем записываем массив в локал 
        if(rateArr.indexOf(el.id) == -1) {
            rateArr.push(el.id);
            localStorage.setItem(`rate`, rateArr);
        }      
    });
});
//console.log(rates);
document.addEventListener('DOMContentLoaded', () => { // выводим при загрузке страницы сохраненный рейтинг
    i = 0;
    r = 0;
    f = 0;
...
//(arrRate.lastIndexOf(el.id) && arrRate.lastIndexOf(el.id) != -1) == true)


// ratingSys.forEach( el => {
//     el.addEventListener('click', (event) => {
//         for(let i = 0; i < ratingSys.length; i++) {
//             let local;
//             if(event.currentTarget.name == `rate${i}`) {
//                 local = `rate${i}`;
//             }
//             //let parent = ratingSys[i];
//             let childLAbels = ratingSys[i].querySelectorAll(`[type='radio']`);
//             childLAbels.forEach(hart => {
//                 if(hurt.checked) {
//                     localStorage.setItem(local, hurt.id);
//                 }
//             });
...
//     let childLabel = el.querySelectorAll(`[type='radio']`);
//     childLabel.forEach(el => {

//     });

//     console.log(childLabel);
//     for(let i = 0; i <= childLabel.length; i++) {
//         childLabel[i].addEventListener('click', (event) => {
//             let local = localStorage.getItem(`rate${i}`)
//                     if(local != null) {
//                         let hartId = [childLabel[i].id];
//                         rateArr.push();
//                         localStorage.setItem(`rate${i++}`, hartId);
//                     } 
//         });
//     }
...
//     childLabel.forEach(hart => {
//        hart.addEventListener('click', () => { // циклом идем по сердечкам, и при клике, если айди кликнутого элемента в массиве нет - добавляем в массив, затем записываем массив в локал 
//            let local = localStorage.getItem(`rate${f}`);
//                    if(local != null) {
//                        let hartId = [hart.id];
//                        rateArr.push(hartId);
//                        localStorage.setItem(`rate${f++}`, hartId);
//                    } 
//        });
//     });
// });